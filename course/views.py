...
def course_single(request, slug):
    ...
    # Thêm logic lấy submission cho sinh viên hiện tại (nếu là sinh viên)
    user_submissions = {}
    if request.user.is_student:
        for assignment in assignments:
            submission = assignment.submissions.filter(student=request.user).order_by('-submitted_at').first()
    ...
    return render(
        ...
        "course/course_single.html",
        {
            ...
            "user_submissions": user_submissions,  # Thêm dữ liệu submission vào context
            "media_url": settings.MEDIA_URL,
        },
    )
...
def submit_assignment(request, assignment_id):
    ...
    if request.method == "POST":
        ...
        # Đặt tất cả các submission cũ của sinh viên cho assignment này thành is_final=False
        Submission.objects.filter(
            assignment=assignment, student=request.user
        ).update(is_final=False)

        # Tạo submission mới với is_final=True
        new_submission = Submission.objects.create(
            assignment=assignment,
            student=request.user,
            uploaded_file=uploaded_file,
            is_final=True  # Đánh dấu là submission cuối cùng
        )
        print(f"DEBUG: Created submission ID {new_submission.id} with is_final=True")
    ...
    # Lấy submission mới nhất của sinh viên (nếu có)
...
def submission_list(request, assignment_id):
    ...
    # Subquery để lấy submission mới nhất của từng sinh viên
    latest_submissions = Submission.objects.filter(
        assignment=assignment,
        student=OuterRef('student')  # Lấy submission liên kết với từng student
    ).order_by('-submitted_at')  # Sắp xếp giảm dần theo thời gian nộp

    submissions = Submission.objects.filter(
        assignment=assignment,
        is_final=True  # Lấy final submissions
    ).select_related('student').order_by('student__username')

    print(f"DEBUG: Found {submissions.count()} submissions with is_final=True")
    return render(
        request, 
        "course/submission_list.html", 
...
def create_assignment(request, course_slug):
    ...
    if request.method == "POST":
        ...
        Assignment.objects.create(
            title=title, description=description, due_date=due_date, course=course
        )
        messages.success(request, "Assignment created successfully.")
        return redirect("assignment_list", course_slug=course.slug)
    return render(request, "course/create_assignment.html", {"course": course})
...
def assignment_add(request, course_slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            # Chuyển hướng về assignment_list với course_slug
            return redirect("assignment_list", course_slug=course.slug)
    else:
        form = AssignmentForm()
    return render(
        request,
        "course/assignment_form.html",
        {"form": form, "course": course, "title": "Add Assignment"},
    )
...
def assignment_edit(request, course_id, assignment_id):
    ...
    return render(
        request,
        "course/assignment_form.html",
        {"form": form, "course": course, "title": "Edit Assignment"},
    )
...
def assignment_delete(request, course_id, assignment_id):
    ...
    if request.method == "POST":
        ...
        # Chuyển hướng về danh sách assignment của course
        return redirect("assignment_list", course_slug=course.slug)

    return render(
        request,
        "course/assignment_delete.html",
        {"assignment": assignment, "course": course, "title": "Delete Assignment"},
    )
